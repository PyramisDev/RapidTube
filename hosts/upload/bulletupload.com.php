...
<?php
	...
	html_error('Extension not allowed for file: "'.$lname.'". These extensions are not allowed: php, pl, cgi, py, sh, shtml');
			...
			$post["file_0_descr"] = '';
			$post['tos'] = '1';
			$upfiles = upfile($url["host"], $url["port"] ? $url["port"] : 80, $url["path"] . ($url["query"] ? "?" . $url["query"] : ""), "http://bulletupload.com/", 0, $post, $lfile, $lname, "file_0");
?>
...
is_page($upfiles);
			...
			$page = geturl("bulletupload.com", 80, "/", 0, 0, $post, 0, $_GET["proxy"]);
				if(!empty($dl[1][0]))
